...
class BMInterfaceTournament extends BMInterface {
    ...
    public function is_tournament_watched($playerId, $tournId) {
        try {
            ...
            $statement->execute(array(':tourn_id' => $tournId,
                                      ':player_id' => $playerId));
    ...
    public function watch_tournament($playerId, $tournId) {
        try {
            ...
            $statement = self::$conn->prepare($query);
            $statement->execute(array(':tourn_id1' => $tournId,
                                      ':player_id1' => $playerId,
                                      ':tourn_id2' => $tournId,
                                      ':player_id2' => $playerId));
    ...
    protected function load_tournament_parameters($tournamentId) {
        ...
        $statement1->execute(array(':tournament_id' => $tournamentId));
    ...
    protected function load_tournament_participants(BMTournament $tournament) {
        ...
        $query = 'SELECT t.* '.
                 ...
                 'ORDER BY t.position;';
        $statement1 = self::$conn->prepare($query);
        $statement1->execute(array(':tournament_id' => $tournament->tournamentId));
    ...
    protected function save_basic_tournament_parameters(BMTournament $tournament) {
        ...
        $statement = self::$conn->prepare($query);
        $statement->execute(array(':status' => $this->get_tournament_status($tournament),
    ...
    protected function save_player_parameters(BMTournament $tournament) {
        foreach ($tournament->playerIdArray as $playerIdx => $playerId) {
            ...
            $statement = self::$conn->prepare($query);
            $statement->execute(array(':position' => $playerIdx + 1,
    ...
    public function dismiss_tourn($playerId, $tournId) {
        try {
            ...
            $statement1 = self::$conn->prepare($query1);
            $statement1->execute(array(
                ':player_id' => $playerId,
                ':tourn_id' => $tournId,
            ));
            $fetchResult = $statement1->fetchAll();
